...
# CHANGE 26 (외부 변경 감지 / 문서 구조)
## `04_Branding/GUNG-ON_굿즈_섹션_정리_ver 1.0.md` 신규 발견 — index.md 미반영 상태였음

세션 시작 프로토콜(`find`로 실제 폴더 대조)에서 index.md에 없던 신규 파일 발견. Claude를 거치지 않고 vault에 직접 추가된 것으로 추정(Figma 섹션 `굿즈`, node `2337:1533`을 읽어 md로 정리한 내용과 동일).

- index.md 파일 목록·한 줄 설명·미해결 사항에 반영 완료.
- **내용 충돌 발견**: 이 굿즈 기획은 GPS(Smart Glass 수집 정보)와 NFC(굿즈 태그·3가지 리마인드 방식 중 1번)를 핵심 전제로 삼고 있음. 그런데 index.md "사용하지 않는 기능"엔 GPS가 명시적으로 금지돼 있고, "Custom Goods" 섹션엔 "NFC 기반 기록 재열람은 현재 핵심 기능으로 확정하지 않는다"고 돼 있어 본 덱 정책과 어긋남. 이 굿즈 안을 정식 채택할지, 정책 쪽을 갱신할지는 사용자 확인 필요 — 임의로 어느 한쪽에 맞춰 고치지 않음.
- Figma 쪽 "변경 로그" 페이지(`WngH1Jt8rwnA6HxQ99mMvv`, page `80:2`)에는 이 섹션 관련 항목이 전혀 없음 — 트래킹되는 17장 본 덱과는 별개의 실험/스크래치 영역으로 보임(좌표상으로도 본 덱과 멀리 떨어진 위치).
...
# CHANGE 25 (문서 구조 / 외부 변경 감지)
## 09-08~09-12 외부 변경 일괄 반영 — 폴더 리네임 1건, 신규 폴더 1개, 신규 문서 8개

세션 시작 시 실제 파일 시스템과 `index.md`를 대조해 발견한 변경(Claude를 거치지 않고 Obsidian에서 직접 작업한 것):

1. `05_research/` → **`06_research/`**로 리네임 — `05_Presentation`과의 번호 중복이 이걸로 해소됨.
2. **`07_raw/` 신설**(2026-09-12) — 정제 전 원본 보관용. `07_raw/3주차/`에 `GUNG ON 검증 파트.md`, `서비스디자인 보완서.md`, 그리고 `01_3주차 전체 기획 정리/`(`_ver.1.0`, `VER 2.0`, `VER 2.1`, `VER 2.3` 4개).
3. `02_Service_Design/`에 **`GUNGON_Service_Planning_VER 2.0.md`**(09-12 15:39)와 **`GUNGON_Service_Planning_VER_2.1.md`**(09-12 19:17) 추가 — VER 1.0도 그대로 남아 세 버전 공존.
4. `04_Branding/00_Index/페르소나체험형_경복궁관람시스템_브랜딩_가이드_v1_0.md` 09-08 수정됨.

### VER 2.1이 VER 2.0과 다른 점 (섹션 기준)

- "대표 프로그램 축소" 섹션 추가
- 경험 단계 명칭 정리: 준비 · **시선** · 체험 · **기억** (2.0의 "해석 / 기록·확장"을 대체)
- 8장을 "터치포인트 기준 + 경험 단계 기준" 두 축 정리로 교체(2.0은 "서비스 설명과 상세 설계의 기준")
- 용어를 영문 혼용에서 한글 표기로 통일(스마트 글라스 / 인터랙티브 스크린 / 웹)

### 확인 필요(아직 미정, index.md 미해결 사항에도 기록)

- `VER_2.1` 파일명이 언더스코어라 컨벤션(`VER 2.1`)과 다르고, 문서 안 제목 줄은 여전히 "(VER 2.0)" — 정정 여부 사용자 확인 대기
- VER 1.0 / 2.0을 `_archive_{날짜}/`로 옮길지 여부

### Figma 쪽에서 같이 확인된 사항(이 vault 변경 아님, 참고)

기본 작업 파일(`WngH1Jt8rwnA6HxQ99mMvv`)에 Figma "변경 로그" 페이지에는 없는 변경이 있음: 신규 페이지 `9/15_3주차`(슬라이드 1·2·21·22·23·24), 기존 `Page 8`이 `3주차 로고 / 무드보드`로 리네임되고 섹션 `로고_1`·`로고_2`·`웹`·`굿즈`·`제품` 추가, 구분선용 빈 페이지 1개 추가. 2026-09-12 로고 SVG 작업도 Figma 변경 로그에는 항목이 없음.
